$(function(){
    ...
    function Region(province,city,region){
        function init(p,c,r){
            if(p){
                var city = 0
                $('.region .select').eq(0).find('ul').html('')
                provinceList.forEach(function(ele,ind){
                    $('.region .select').eq(0).find('ul').append(`<li>${ele.name}</li>`)
                    if(ele.name === p){
                        city = ind
                    }
                })
                var area = 0
                if(!c){
                    $('.region .select input').eq(1).val(provinceList[city].cityList[0].name) 
                    $('.region .select .text').eq(1).html(provinceList[city].cityList[0].name)
                } 
                
                $('.region .select').eq(1).find('ul').html('')
                provinceList[city].cityList.forEach(function(ele,ind){
                    $('.region .select').eq(1).find('ul').append(`<li>${ele.name}</li>`)
                    if(c && ele.name === c){
                        area = ind
                    }
                })
                if(!r){
                    $('.region .select input').eq(2).val(provinceList[city].cityList[area].areaList[0]) 
                    $('.region .select .text').eq(2).html(provinceList[city].cityList[area].areaList[0])
                }
                
                $('.region .select').eq(2).find('ul').html('')
                provinceList[city].cityList[area].areaList.forEach(function(ele,ind){
                    $('.region .select').eq(2).find('ul').append(`<li>${ele}</li>`)
                })
                ...
                provinceList[0].cityList.forEach(function(ele,ind){
                    $('.region .select').eq(1).find('ul').append(`<li>${ele.name}</li>`)
                })
                provinceList[0].cityList[0].areaList.forEach(function(ele,ind){
                    $('.region .select').eq(2).find('ul').append(`<li>${ele}</li>`)
                })
        ...
        function bindEvent(){
            $('.region .select input').eq(0).on('change',function(e){
                init(e.target.value)
            })
            $('.region .select input').eq(1).on('change',function(e){
                init($('.region .select input').eq(0).val(),e.target.value)
            })
        }
        init(province,city,region)

    }
    var province = $('.region .select input').eq(0).val()
    var city = $('.region .select input').eq(1).val()
    var region = $('.region .select input').eq(2).val()

    var reg = new Region(province,city,region)
