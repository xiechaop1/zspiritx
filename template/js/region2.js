$(function(){
    ...
    function Region(province,city){
        $('.region .select select').eq(0).html('')
        provinceList.forEach(function(ele,ind){
            $('.region .select select').eq(0).append(`<option value="${ele.name}">${ele.name}</option>`)
        })
        ...
        function init(p,c){
            if(p){
                ...
                var area = 0
                if(!c){
                    $('.region .select select').eq(1).val(provinceList[city].cityList[0].name)
                }
                provinceList.forEach(function(ele,ind){
                    if(ele.name === p){
                        city = ind
                ...
                $('.region .select select').eq(1).html('')
                provinceList[city].cityList.forEach(function(ele,ind){
                    $('.region .select select').eq(1).append(`<option value="${ele.name}">${ele.name}</option>`)
                    if(c && ele.name === c){
                        area = ind
                    }
                })
        ...
        function bindEvent(){
            $('.region .select select').eq(0).on('change',function(e){
                init(e.target.value)
            })
        }
        init(province,city)
    ...
    var province = $('.region .select select').eq(0).val()
    var city = $('.region .select select').eq(1).val()
    //var region = $('.region .select input').eq(2).val()

    var reg = new Region(province,city)
