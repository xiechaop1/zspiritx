...
(function ($) {
  ...
  fn.createCalendar = function () {
    ...
    // 上一月按钮
    if (showPrevBtn)
      html += '       <a class="prev-month  " title="上一月"><i class="fs-12 iconfont iconicon-rilizuojiantou"></i>'+ ''+'</a>';
    // 标题
    html += '		<div class="calendar-month-title">'+ formatDate(this.month, 'yyyy年MM月') +'</div>';
    // 下一月按钮
    if (showNextBtn)
      html += '       <a class="next-month" title="下一月"><i class="fs-12 iconfont iconicon-riliyoujiantou"></i>'+ '' +'</a>';
    ...
    for (var i = 0; i < rows; i++){
      ...
      for (var k = 1; k <= 7; k++){
        ...
        if (d > 0 && d <= thisMonthDays){
          ...
          var me = this;
          var dayData = null;
          dayData = me._getDateData(tdId);

          var stock=0;
          if(dayData){
            var stock = dayData.stock+1;
            var status=dayData.groupStatus;
          }
          ...
          if (tdId >= startDay && tdId <= endDay ) {
            if(stock>1 && status==1){
              html += '<td class="valid-hook" data-week="' + (k - 1) + '" data-id="' + tdId + '"><div class="valid-box"><b class="text-33">' + d + '</b><div class="data-hook"></div></div></td>';
            }
            else if(status==2) {
              html += '<td class="disabled" data-week="' + (k - 1) + '" data-id="' + tdId + '"><div class="valid-box"><b class="text-33">' + d + '</b><div class="data-hook"></div></div></td>';
            }
            else if(status==0){
              html += '<td class="disabled" data-week="' + (k - 1) + '" data-id="' + tdId + '"><div class="valid-box"><b class="text-33">' + d + '</b><div class="data-hook"></div></div></td>';
...
*/
  ...
  fn.renderDataToTalbe = function () {
    ...
    this.calendar.find('.valid-hook').each(function () {
      ...
      html = me.dayTemplate().toString();
      if (dayData) {
        var stock=dayData.stock;
        var price=dayData.price;
        var status=dayData.groupStatus;

        if(stock>0 && status==1){
          if(stock>9){
            html='<p class="text-F6">£'+ price +'</p>'+'<p class="text-F6">充足</p>'

          }
          else {
           // html='<p class="text-F6">£'+ price +'</p>'+'<p class="text-99">余<span class="text-F6">'+ stock+'</span>座</p>'
            html='<p class="text-F6">£'+ price +'</p>'+'<p class="text-F6">余<span class="text-F6">'+ stock+'</span>座</p>'
          ...
          html='<p class="text-F6"> </p>'+'<p class="text-99">已售罄</p>'

        }else if(status==0) {
          html='<p class="text-F6"> </p>'+'<p class="text-99">待售</p>'
        }
        else{
          html='<p class="text-F6"> </p>'
        }
  ...
  fn.dayTemplate = function () {
    ...
    if (arr && arr instanceof Array) {
      for (var i = 0; i < arr.length; i++) {
        ...
        if(val.name=='余座'){
          html += '<p class="text-99">余<span class="text-F6">'+ '{'+ val.key +'}</span>座</p>'
  ...
  fn.handleClickEvent = function () {
    ...
    this.calendar.on('click', '.valid-hook', function () {
      ...
      var dayData = null;
      dayData = me._getDateData(currentDay);

      var adultMax,adultMin,childMax,childMin,adultPrice,adultPrice2,childPrice,childPrice2,groupId;
      if(dayData){
        dayData.buyAdultMax>0?adultMax=dayData.buyAdultMax:adultMax=0;
        // dayData.buyAdultMax>10?adultMax=10:adultMax=dayData.buyAdultMax;
        dayData.buyAdultMin>0?adultMin=dayData.buyAdultMin:adultMin=0;
        dayData.buyChildMax>0?childMax=dayData.buyChildMax:childMax=0;
        dayData.buyChildMax>10?childMax=10:childMax=dayData.buyChildMax;
        dayData.buyChildMin>0?childMin=dayData.buyChildMin:childMin=0;
        dayData.adultPrice>0?adultPrice=dayData.adultPrice:adultPrice=0;
        dayData.adultPrice2>0?adultPrice2=dayData.adultPrice2:adultPrice2=0;
        dayData.childPrice>0?childPrice=dayData.childPrice:childPrice=0;
        dayData.childPrice2>0?childPrice2=dayData.childPrice2:childPrice2=0;
        dayData.groupId>0?groupId=dayData.groupId:groupId="";
      }
      else {
        adultMax=0;
        adultMin=0;
      ...
      $(".valid-hook").removeClass("active");
      $(activeItem).addClass("active");

      $('.boutique-detail-right .datepicker').val(currentDay)

      $(".boutique-detail-right input[name='adultnumber'].boutique").attr('min',adultMin).attr('max',adultMax).attr('data-adult-price',adultPrice).attr('data-adult-china-price',adultPrice2);
      $(".boutique-detail-right input[name='childcounter'].boutique").attr('min',childMin).attr('max',childMax).attr('data-adult-price',childPrice).attr('data-adult-china-price',childPrice2);
      $(".boutique-detail-right input[name='group_id']").val(groupId);
      console.log("groupId:"+ $(".boutique-detail-right input[name='group_id']").val())

      var priceHtml=' <span class="text-66">￡</span><span class="fs-28 bold">'+adultPrice+'</span>\n' +
        '<span class="text-66">/￥'+adultPrice2+'人起</span>';
      $(".boutique-detail-right .price").empty().html(priceHtml);


      var adult= parseInt($(".boutique-detail-right input[name='adultnumber'].boutique").val());
      var child= parseInt($(".boutique-detail-right input[name='childcounter'].boutique").val());

      if(adult>adultMax){
        $(".boutique-detail-right input[name='adultnumber'].boutique").val(adultMax)
      }
      if(child>childMax){
        $(".boutique-detail-right input[name='childcounter'].boutique").val(adultMax)
      }

      boutiqueDetailPriceSum()
...
/*
    ...
    //显示价格
    function boutiqueDetailPriceSum() {
      var adult=$("input[name='adultnumber'].boutique");
      var adultNum= parseInt(parseInt(adult.val()));
      var adultPrice=parseFloat(adult.attr('data-adult-price'));
      var adultChinaPrice=parseFloat(adult.attr('data-adult-china-price'));
      adultNum>=0?adultNum=adultNum:adultNum=0;
      adultPrice>=0?adultPrice=adultPrice:adultPrice=0;
      adultChinaPrice>=0?adultChinaPrice=adultChinaPrice:adultChinaPrice=0;

      console.log(adultPrice+";"+adultNum+";"+adultChinaPrice)
      var child=$("input[name='childcounter'].boutique");
      var childNum= parseInt(child.val());
      var childPrice=parseFloat(child.attr('data-child-price'));
      var childChinaPrice=parseFloat(child.attr('data-child-china-price'));
      childNum>=0?childNum=childNum:childNum=0;
      childPrice>=0?childPrice=childPrice:childPrice=0;
      childChinaPrice>=0?childChinaPrice=childChinaPrice:childChinaPrice=0;
      console.log(childNum+";"+childChinaPrice+";"+childPrice)

      var price1=(adultNum*adultPrice+childNum*childPrice).toFixed(2);
      price1>0?price1=price1:price1='0.00';
      var price2=(childNum*childChinaPrice+adultNum*adultChinaPrice).toFixed(2);
      price2>0?price2=price2:price2='0.00';
      var price='    <span class="fs-12 text-66">￡</span>\n' +
        '                                <span class="fs-26 bold color-orange">'+price1+'</span>\n' +
        '                                <span class="fs-12 text-66">/</span>\n' +
        '                                <span class="fs-14 color-orange">￥'+price2+'</span>'

      $(".boutique-price-sum").html(price);
    }
  ...
  fn.createStyleCode = function () {
    ...
    var defaultStyle = {
      ...
      // Hover
      validDateHoverBgColor: '',
      validDateHoverTextColor: '#F66D57',
      // 无效日期颜色
